...
std::vector<SystemMetrics> MetricsHistory::tail(size_t n) {
    ...
    if (!fs::exists(current_file)) return result;
    ...
    for (size_t i = start; i < lines.size(); ++i) {
        ...
        SystemMetrics m;
        char comma;
        iss >> m.timestamp >> comma >> m.cpu_usage_percent >> comma 
            >> m.mem.total >> comma >> m.mem.free >> comma >> m.mem.available;
        result.push_back(m);
    }
...
SystemMetrics MetricsSampler::sample() {
    ...
    if (!first_sample_) {
        uint64_t prev_total = prev_cpu_.user + prev_cpu_.nice + prev_cpu_.system + 
                              prev_cpu_.idle + prev_cpu_.iowait + prev_cpu_.irq + prev_cpu_.softirq;
        uint64_t current_total = current.user + current.nice + current.system + 
