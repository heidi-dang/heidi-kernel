...
class KernelClient {
public:
  std::string query(const std::string& cmd, const std::string& body = "") {
    ...
    if (send(fd_, request.c_str(), request.size(), MSG_NOSIGNAL) < 0) {
      close_connection();
      if (!connect_to_kernel()) {
        return "{\"error\":\"kernel_not_running\"}";
      }
    ...
    char buf[4096];
    while (true) {
      ssize_t n = recv(fd_, buf, sizeof(buf), MSG_PEEK);
      if (n <= 0) {
        close_connection();
