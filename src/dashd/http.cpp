...
void HttpServer::serve_forever() {
  ...
  while (true) {
    // Reap finished futures to free up slots
    ...
    try {
      futures.push_back(std::async(std::launch::async, [this, client_fd] {
        // RAII wrapper to ensure fd is closed exactly once when this scope exits
        struct FdGuard {
          int fd;
          ~FdGuard() {
            if (fd >= 0)
        ...
        try {
          handle_client(guard.fd);
        } catch (...) {
          // Exception in handler, guard will close fd
        }
      }));
    } catch (const std::system_error&) {
      const char* resp_500 =
          "HTTP/1.1 500 Internal Server Error\r\nContent-Length: 0\r\nConnection: close\r\n\r\n";
      write(client_fd, resp_500,
...
void HttpServer::handle_client(int client_fd) {
  ...
  while (true) {
    if (request_buffer.size() >= MAX_REQUEST_SIZE) {
      ...
      write(client_fd, resp_413,
            sizeof("HTTP/1.1 413 Payload Too Large\r\nContent-Length: 0\r\n\r\n") - 1);
      return;
    }
