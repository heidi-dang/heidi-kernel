...
EventLoop::EventLoop(std::chrono::milliseconds tick_interval) : tick_interval_(tick_interval) {}
...
void EventLoop::run() {
  ...
  worker_ = std::thread([this]() { tick_loop(); });
}
...
void EventLoop::tick_loop() {
  ...
  while (!stop_requested_.load()) {
    ...
    cv_.wait_for(lock, std::chrono::milliseconds{10}, [this] { return stop_requested_.load(); });
  }
