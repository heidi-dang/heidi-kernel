...
ResourceGovernor::ResourceGovernor(const GovernorPolicy& policy)
    : policy_(policy) {
}

GovernorResult ResourceGovernor::decide(double cpu_pct, double mem_pct, int running_jobs, int queued_jobs) const {
...
PolicyUpdateResult ResourceGovernor::validate_and_update(const GovernorPolicy& policy) {
    ...
    if (std::isnan(policy.cpu_high_watermark_pct) || policy.cpu_high_watermark_pct < 0.0 || policy.cpu_high_watermark_pct > 100.0) {
        result.errors.push_back({"cpu_high_watermark_pct", "must be between 0 and 100"});
        result.success = false;
    }

    // Validate mem_high_watermark_pct
    if (std::isnan(policy.mem_high_watermark_pct) || policy.mem_high_watermark_pct < 0.0 || policy.mem_high_watermark_pct > 100.0) {
