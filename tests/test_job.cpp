...
// Include the procfs starttime helper for a small unit test below.
#include "../src/job/procfs_starttime.h"
...
protected:
  void SetUp() override {
    ...
    job_runner_ =
        new heidi::JobRunner(20, spawner_, inspector_); // Higher concurrency for budget tests
    job_runner_->start();
  }
...
TEST_F(JobTest, GetJobStatusAfterSubmit) {
  ...
  EXPECT_TRUE(job->status == heidi::JobStatus::QUEUED || job->status == heidi::JobStatus::RUNNING ||
              job->status == heidi::JobStatus::STARTING);
}
...
TEST_F(JobTest, JobCompletesSuccessfully) {
  ...
  EXPECT_EQ(job_runner_->get_last_tick_diagnostics().last_decision,
            heidi::GovernorDecision::START_NOW);
...
TEST(ParseStartTime, HandlesCommWithSpaces) {
  // Build a synthetic /proc/<pid>/stat line where comm contains spaces and
  // the starttime (field 22) is 999999. We include enough fields after ')' to
  // ensure the parser can find field 22.
