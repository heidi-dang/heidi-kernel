...
TEST_F(IntegrationTest, IT_RunningCap_HoldsStarts) {
    ...
    for (int i = 0; i < 15; ++i) {
        job_ids.push_back(job_runner_->submit_job("/home/heidi/heidi-kernel/build/tests/integration/hk_sleep --seconds 60"));
    }
...
TEST_F(IntegrationTest, IT_Timeout_KillsProcessGroup) {
    ...
    std::string job_id = job_runner_->submit_job("/home/heidi/heidi-kernel/build/tests/integration/hk_sleep --seconds 60", limits);
...
TEST_F(IntegrationTest, IT_ProcCap_KillsProcessGroup) {
    ...
    std::string job_id = job_runner_->submit_job("/home/heidi/heidi-kernel/build/tests/integration/hk_forkbomb_safe --children 10 --hold-ms 60000", limits);
    ...
    for (int i = 0; i < 10; ++i) {
        ...
        job_runner_->tick(now_ms, metrics);
        job = job_runner_->get_job_status(job_id);
        if (job->status == JobStatus::PROC_LIMIT) break;
    }
