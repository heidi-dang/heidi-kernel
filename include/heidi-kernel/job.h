...
class JobRunner {
public:
  JobRunner(size_t max_concurrent_jobs = 10, IProcessSpawner* spawner = nullptr,
            IProcessInspector* inspector = nullptr);
  ~JobRunner();
  ...
  const TickDiagnostics& get_last_tick_diagnostics() const {
    return last_tick_diagnostics_;
  }
  size_t get_jobs_started_this_tick() const {
    return jobs_started_this_tick_;
  }
  size_t get_jobs_scanned_this_tick() const {
    return jobs_scanned_this_tick_;
  }

  // Limit enforcement methods
  void tick(uint64_t now_ms, const SystemMetrics& metrics, size_t max_starts_per_tick = 5,
